...
function renderForecast(card, data) {
  ...
  card.querySelector('.current .uvindex .value')
      .textContent = Math.round(data.currently.uvIndex);
  ...
  card.querySelector('.current .wind .direction')
      .textContent = getCardinalDirection(Math.round(data.currently.windBearing));
  ...
  futureTiles.forEach((tile, index) => {
    const forecast = data.daily.data[index];
    console.log('index = '+index+' forecast = '+forecast.time);
    //const forecast = data.daily.data[index];
    ...
    console.log('forecastFor='+forecastFor);
    
    //massive hack - looks like luxon gets mixed up  - patterns like EEE cionsistently produce the wrong name of the day, yet full pattern like DDDD get it right but are too long to display
    
   
    
switch(index) {
  case 0:
    tile.querySelector('.date').textContent = 'Today';
    console.log('Todays forecast:' +data.daily.summary);
    card.querySelector('.forecast').textContent = data.daily.summary;
    break;
  case 1:
    tile.querySelector('.date').textContent = 'Tomorrow';
    break;
  default:
    tile.querySelector('.date').textContent = forecastFor.substr(0,3);
  }
...
function getForecastFromNetwork(coords) {
  return fetch(`/forecast/${coords}?units=si`)
      .then((response) => {
...
const url = `${window.location.origin}/forecast/${coords}?units=si`;
...
function loadLocationList() {
  ...
  if (!locations || Object.keys(locations).length === 0) {
    ...
    locations = {};
    locations[key] = {label: 'Melbourne', geo: '-37.8136,144.9631'};
  }
  return locations;
}
...
function getCardinalDirection(angle) {
  ...
  //reversed original direction arrows to represent wind bearing
